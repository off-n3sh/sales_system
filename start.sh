...
mongo_auth_required() {
  # Returns:
  # 0 => auth required
  # 1 => auth NOT required (open)
  # 2 => unreachable/other error
  local host="$1"
  local out rc
...
# Canonical conf path for this deployment
...
conf_is_fully_configured() {
  # Returns 0 (true) only if auth is enabled AND bindIp is already set to something
...
create_mongod_conf() {
  # Writes a clean minimal mongod.conf with auth enabled and bindIp 0.0.0.0.
  # Creates parent directory if needed. Backs up any existing file first.
  ...
  info "Writing new mongod.conf: bindIp=0.0.0.0, authorization=enabled"
...
apply_mongod_conf_auth_bind() {
  # Only for LOCAL Mongo.
  # If fully configured already -> show current conf, pause, return.
  # If conf missing or incomplete -> create/overwrite with canonical conf.
  need_cmd mongod || { warn "mongod not installed; skipping mongod.conf changes"; return 0; }

  if conf_is_fully_configured; then
    ok "mongod.conf already configured (authorization enabled, bindIp set)."
    echo
  ...
  warn "mongod.conf missing or incomplete — creating canonical conf."
...
step_mongo_config_users_seed() {
  ...
  # Local: fix conf before reachability check — bad/missing conf may be why mongod is down
  if [[ "$is_local" =~ ^[Yy]$ ]]; then
  ...
  # Auth check — this decides the entire path below
  local auth_required
  ...
  # ── PATH A: Auth enforced — instance is already set up ───────────────────
  # Do NOT create or seed anything. Just verify the creds and cache them.
...
EOF"
  ...
  # ── PATH B: Auth not enforced — fresh install ─────────────────────────────
...
step_env_file() {
  ...
  # Pull cached values from step 3 if available
  ...
  # Always prompt with defaults shown — works standalone on a pre-configured host.
  echo
...
EOF"
  ...
  # root owns it, no other user can read it
